...
class AboutControlStatements < Neo::Koan
  ...
  def test_unless_statement_modifier
    result = :default_value
    result = :false_value unless false # same as saying "while true" or if true.
  ...
  def test_break_statement_returns_values
    i = 1
    result = while i <= 10
               break i if i.even? # break out of the loop and return the value of i
  ...
  def test_next_statement
    ...
    while i < 10
      i += 1
      next if i.even?   # skip the rest of the loop and go to the next iteration
