...
class AboutScope < Neo::Koan
  ...
  def test_nested_string_is_not_the_same_as_the_system_string
    assert_equal false, 'HI'.instance_of?(String)
  end

  def test_use_the_prefix_scope_operator_to_force_the_global_scope
    assert_equal true, 'HI'.instance_of?(::String)
  end
  ...
  def test_class_names_are_just_constants
    ...
    assert_equal true, 'HI'.instance_of?(MyString)
  end

  def test_constants_can_be_looked_up_explicitly
    assert_equal true, AboutScope.const_get('PI') == PI
    assert_equal true, AboutScope.const_get('MyString') == MyString
  end
